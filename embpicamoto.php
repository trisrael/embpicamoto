		...
		if(!empty($options['embpicamoto_options_login']) && !empty($options['embpicamoto_options_password'])) {
			try {
				...
				if($has_pages){ #Add in jQuery tabs conditionally
					...
					$script = $script . "while(!isLoaded){ try{ if(jQuery && jQuery.ui && jQuery.ui.tabs){ jQuery('#$wrap_el_id').tabs(); isLoaded= true;} alert('made it');}catch(err){alert('Javascript libary missing: jQuery UI tabs has not loaded properly')} }})();";
